console.log('initializing fio extension');
chrome.extension.sendRequest("show_page_action");

$(document).ready(function () {
    if ($("#product-detail-description").length)         // use this if you are using id to check
    {
        var productID = $('[data-sel~="catalog-number"]').text();
        $("<div class=\"rsc\">" +
            "    <div class=\"rsc-container sc-iwsKbI kdZMGe\" style=\"width: 280px;\" width=\"350px\">" +
            "        <div class=\"rsc-content sc-gZMcBi bxslzG\" style=\"overflow: hidden;\">" +
            "            <div class=\"rsc-ts sc-dnqmqq efROPc\">" +
            "                <div class=\"rsc-ts-image-container sc-htoDjs vmYlS\"><img class=\"rsc-ts-image sc-gzVnrw hLGSaN\"" +
            "                                                                         src=\"data:image/svg+xml;base64,\"" +
            "                                                                         alt=\"avatar\"></div>" +
            "                <div class=\"rsc-ts-bubble sc-bZQynM fhAWVE\">Welcome to chatbot!</div>" +
            "            </div>" +
            "            <div class=\"rsc-ts sc-dnqmqq efROPc\">" +
            "                <div class=\"rsc-ts-image-container sc-htoDjs vmYlS\"></div>" +
            "                <div class=\"rsc-ts-bubble sc-bZQynM jgMjlR\">Check out the documentation!</div>" +
            "            </div>" +
            "            <div class=\"rsc-os sc-EHOje edRFCv\">" +
            "                <ul class=\"rsc-os-options sc-ifAKCX gkhNlr\">" +
            "                    <li class=\"rsc-os-option sc-htpNat hvWyFD\"><a class=\"rsc-os-option-element sc-bxivhb fMcZCH\">Ano</a></li>    " +
            "                    <li class=\"rsc-os-option sc-htpNat hvWyFD\"><a class=\"rsc-os-option-element sc-bxivhb fMcZCH\">Ne</a></li></ul>" +
            "            </div>" +
            "        </div>" +
            "        <div class=\"rsc-footer sc-jzJRlG byHcWR\" style=\"display: none;\"><input class=\"rsc-input sc-cSHVUG kCSgO\"" +
            "                                                                               placeholder=\"Type the message ...\"" +
            "                                                                               value=\"\" disabled=\"\" type=\"textarea\">" +
            "            <button class=\"rsc-submit-button sc-kAzzGY kZiwDc\" disabled=\"\">" +
            "                <svg version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" width=\"20\" height=\"20\" viewBox=\"0 0 500 500\">" +
            "                    <g>" +
            "                        <g>" +
            "                            <polygon points=\"0,497.25 535.5,267.75 0,38.25 0,216.75 382.5,267.75 0,318.75\"></polygon>" +
            "                        </g>" +
            "                    </g>" +
            "                </svg>" +
            "            </button>" +
            "        </div>" +
            "    </div>" +
            "</div>").appendTo(document.body);
        //todo: post backend id product
        //todo: backend return ( image, url, motivationText )
        (function () {
            var apiUrl = "https://api.logoman.cz/api/product/" + productID;

            $.getJSON(apiUrl, {
                //format: "json"
            })
                .done(function (data) {
                    addSmallBox(data.url, data.image, data.motivationText, data.price);
                });
        })();

    }

});

function addSmallBox(url, image, motivationText, price) {

    var box = "<ul class=\"promo-boxes-wrapper\" style=\"margin-top: 10px\">" +
        "<li class=\"promo-box\">" +
        "<a style=\"background-image: url(" + image + ");\" class=\"promo-box-head\" href=\"" + url + "\"></a>" +
        "<a class=\"link--primary promo-box-title\" style=\"font-color:black;line-height: 1.4; letter-spacing: -0.01em; font-size: 1.1em;\" analytics-label=\"931819\" analytics-event=\"Box1\" analytics-category=\"TV - kampaň\" analytics-on=\"click\" href=\"" + url + "\">" +
        "<span class=\"link--secondary\" style=\"text-decoration:underline\">" + motivationText +
        "</span>" +
        price +
        "</a></li>";

    $('form .detail-buttons').after(box);
}

